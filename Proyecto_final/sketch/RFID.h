...
class rfid {
  ...
  public:
    ...
    byte uidAutorizado3[4] = {0x08, 0x02, 0x9A, 0xEC};
...
void rfid::verificar_tarjeta_presente() {
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    ...
    if (verificar_tarjeta_autorizada(mfrc522.uid.uidByte, mfrc522.uid.size)) {
      ...
      ACTUADORES.show_LCD("Bienvenido", " a casa");
      digitalWrite(MICROSD_PIN, HIGH); 
      MSD.SaveFile(obtener_uid(mfrc522.uid.uidByte, mfrc522.uid.size)); 
      MSD.ReadFile();
      digitalWrite(MICROSD_PIN, LOW); 
    }
...
bool rfid::verificar_tarjeta_autorizada(byte *uid, byte bufferSize) {
  ...
  return (memcmp(uid, uidAutorizado1, sizeof(uidAutorizado1)) == 0 || memcmp(uid, uidAutorizado2, sizeof(uidAutorizado2)) == 0 || memcmp(uid, uidAutorizado3, sizeof(uidAutorizado3)) == 0);
}
