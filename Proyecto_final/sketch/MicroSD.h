...
#define MICROSD_PIN 4    /* Pin de control MicroSD */  
...
void MicroSD :: SaveFile ( String id ) {
    
    // Abrir o crear en caso de que no exista.
    MicroSD_File = SD.open ( filename, FILE_APPEND );
    if ( MicroSD_File ) {
        Myrtc.get_time();
        String json = Json.Acceso_Json(Myrtc.format_date(),Myrtc.format_time(), id);
        MicroSD_File.println(json);
        MicroSD_File.close ( );
    } else {
